...
from src.utils import (down_local, FAO_PET, gee_static,
                       local_dynamic, local_static, ecoregions)
...
def prepare(cfg):
    ...
    dfs = [pd.read_excel(cfg.gages, usecols=['STAID'], dtype=str)]
    ...
    # 1.6. Assign each basin into nine ecoregions
    ecoregions.assign(cfg)

    # 2. preprocess static data from local disk and GEE
    if os.path.exists(cfg.csv.sta):
        return
